...
func testLookPath(s string) ([]string, []string, error) {
	return []string{os.Args[0], "-test.run=TestHelperProcess", "--", s}, []string{"INSTILL_WANT_HELPER_PROCESS=1"}, nil
}

func TestHelperProcess(t *testing.T) {
	if os.Getenv("INSTILL_WANT_HELPER_PROCESS") != "1" {
		return
	}
...
func Test_InstillEditor_Prompt_skip(t *testing.T) {
	pty := newTerminal(t)
...
func Test_InstillEditor_Prompt_editorAppend(t *testing.T) {
	pty := newTerminal(t)
...
func Test_InstillEditor_Prompt_editorTruncate(t *testing.T) {
	pty := newTerminal(t)
